...
3. **EC2 Instance Roles Demo**
   ...
   - So now we are going to see how to provide AWS credentials to our EC2 Instances using IAM Roles.

   - So first of all, we need to create an IAM Role. So go to the IAM service, then click on Roles on the left-hand side, and then click on Create role.

   - So we need to choose a trusted entity. So here we have different options. We can choose AWS service, another AWS account, web identity, or SAML 2.0 federation.

   - So here we have to choose the type of trusted entity. So we are going to choose AWS service because we want to provide AWS credentials to an AWS service, which is EC2 in our case. So select EC2 and then click on Next.
...
5. **SHARED RESPONSIBILITY MODEL FOR EC2**
    ...
    - In the shared responsibility model for EC2, AWS is responsible for the security "of" the cloud, while the customer is responsible for security "in" the cloud.
   ...
   **ME**:
    ...
    - Diagram:
      ...
      |      AWS            |        ME           |
      +---------------------+---------------------+
      | Physical Security   | OS Patching        |
      | Network Security    | Application Setup  |
      | Hardware Maintenance| Data Security      |
      | Hypervisor Security | IAM Roles          |
      +---------------------+---------------------+
      ```
